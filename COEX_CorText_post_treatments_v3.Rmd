---
title: "Retraitement du corpus COEX traité par CorText"
author: "G Ollivier"
...
```{r loading_function, message=FALSE, warning=FALSE, include=FALSE, echo=F}
...
```{r cortext_db_macro, message=FALSE, warning=TRUE,  echo=F}
...
step1 <- get_cortext_corpus('https://assets.cortext.net/docs/05b656f701fa88439367f5d1699987a5', corpus='tout')  # n=500, standard
...
most_representative_doc_by_cluster <- most_representative_doc_by_cluster %>% 
  full_join(step1$corp  %>% select(id_doc_cortext, macro_cluster_label_new) , by=c(id_doc_cortext='id_doc_cortext'))
...
### Sources
```{r sources,fig.caption='Top sources and their type', fig.height=7, fig.width=6}
top=40
g_so <- corp_cortext %>% dplyr::group_by(src, typsrc_en) %>% dplyr::summarise(n=n()) %>% arrange(desc(n)) %>% head(top)  %>% 
  ggplot(aes(x=reorder(src, n), y=n, fill=reorder(typsrc_en, n)) )   + geom_bar( stat='identity') + coord_flip() + 
...
ggarrange(g_so, g_typ_so, common.legend = T, widths=c(3,1), legend='bottom')   
   # pb sur legende
```

  

```{r source_type_dyn, fig.caption='Temporal distribution of source types'}

corp_cortext  %>% ggplot() + geom_bar(aes(x=year_month, fill=  typsrc_en  ), stat='count', position='fill') + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 0))  # mieux ordonner ?
```
...
most_representative_doc_by_cluster   %>% select(- id_doc_cortext, - id_doc_origin , -suprathematique) %>%
  DT::datatable(selection ='multiple', style='default', class = 'cell-border stripe',   filter = 'top', options=list(searchHighlight = TRUE, regex = T, caseInsensitive = T, autoWidth = F, digits=1) ) %>% DT::formatRound(columns='doc2cluster_weight',digits=0) #%>% DT::formatRound(columns='density',digits=2)
...
library(officer)
report <- officer::read_docx() %>% body_add_par(value='Results', style='heading 1') %>% 
  body_add_par(value='Features coming from treatments of CorText outputs with R.')
  body_add_break() %>% body_end_section_landscape()

report <- report %>% body_add_par(value='Macro themes', style='heading 2') %>% 
      body_add_table(macro_synth, style = "table_template", font.size = 9) %>% body_add_break()

# Les principaux textes par clusters


report <- report %>% body_add_par(value='Most representative texts per clusters', style='heading 2')

for (cl in most_representative_doc_by_cluster$cluster_label) { 
  tab <- most_representative_doc_by_cluster %>% subset(cluster_label==cl) %>% select(date, pays, src, text)
  
  report <- report %>% body_add_par(value=cl, style='heading 3')  %>% 
      body_add_table(  tab , style = "table_template") %>% 
      body_add_break()
}

# Saving the report
print(report, target = "COEX_report.docx")
#################
```